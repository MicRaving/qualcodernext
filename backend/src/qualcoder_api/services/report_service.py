...
async def interrater(session: AsyncSession, coder_a: str, coder_b: str) -> dict:
    """Interrater reliability: two coders, each source x code cell rated
    present/absent. Returns Cohen's Kappa, Krippendorff's Alpha (binary
    nominal) and Gwet's AC1.

    ``coder_a``/``coder_b`` may be coder names or empty (matches any
    non-null owner) — the frontend always passes concrete names.
    """
    if coder_a == coder_b:
        raise ValueError("choose two different coders")

    # source id → set of coded (source, code) cells per coder.
    ...
    # All files both coders touched (union), all codes used by either.
    file_ids = {f for f, _ in cells_a} | {f for f, _ in cells_b}
    code_ids = {c for _, c in cells_a} | {c for _, c in cells_b}
    ...
    both = sum(1 for cell in cells_a & cells_b if cell[0] in file_ids)
    only_a = sum(1 for cell in cells_a - cells_b if cell[0] in file_ids)
    only_b = sum(1 for cell in cells_b - cells_a if cell[0] in file_ids)
    ...
    # Krippendorff's alpha, binary nominal: observed disagreement vs
    # expected disagreement from the marginal proportions.
    do = (only_a + only_b) / n_pairs
    pi1 = (2 * both + only_a + only_b) / (2 * n_pairs)
    de = 1 - (pi1 * pi1 + (1 - pi1) * (1 - pi1))
    krippendorff = 1 - do / de if de != 0 else None
