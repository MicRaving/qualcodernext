...
async def import_merge(
    ...
    """
    from qualcoder_api.services import merge_projects

    if svc.project_path == "" or svc.session_factory is None:
        raise HTTPException(status_code=409, detail="no project is open")
    tmp = svc.project_path + "/_merge_" + (file.filename or "merge.zip")
    with open(tmp, "wb") as out:  # noqa: ASYNC230 - small local temp write
        while chunk := await file.read(1 << 20):
            out.write(chunk)
    try:
        ...
        shutil.rmtree(tmp + "_dir", ignore_errors=True)
