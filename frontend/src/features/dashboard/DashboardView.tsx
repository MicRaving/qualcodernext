  ...
  return (
    <div className="flex h-full items-center justify-center bg-bg">
      <div className="w-full max-w-xl px-8">
        <h1 className="text-[28px] font-bold text-text-primary">{t("welcome.title")}</h1>
        <p className="mt-1 text-sm text-text-secondary">{t("welcome.subtitle")}</p>

        {error && (
          <div
            role="alert"
            className="mt-4 rounded-sm border border-danger/50 bg-danger/10 px-3 py-2 text-sm text-danger"
          >
            {error}
          </div>
        )}
              ...
              {recent.map((path) => (
                <li key={path}>
                  <button
                    ...
                    disabled={busy}
                    className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left text-sm hover:bg-surface-higher disabled:opacity-50"
                  >
